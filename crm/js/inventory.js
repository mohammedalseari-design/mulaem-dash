...
export async function renderInventory(root) {
    ...
    replace(root, [
        ...
        el('div', { class: 'crm-card' }, [
            el('div', { class: 'crm-card-head' }, [
                el('h2', { text: 'تغيّرات الأسعار — آخر 30 يومًا' }),
                el('div', { class: 'crm-subtle', text: 'تظهر عند تعديل سعر المشروع' })
            ]),
            priceBody
        ]),
        el('div', { class: 'crm-card' }, [
            el('div', { class: 'crm-card-head' }, [
                el('h2', { text: 'الأولوية للعرض' }),
                el('div', { class: 'crm-subtle', text: 'الأكمل والأحدث أولًا' })
            ]),
            priorityBody
        ]),
        el('div', { class: 'crm-card' }, [
            el('div', { class: 'crm-card-head' }, [
                el('h2', { text: 'فجوة الطلب حسب الحي' }),
                el('div', { class: 'crm-subtle', text: 'الطلبات المفتوحة مقابل العقارات المتاحة' })
            ]),
            gapBody
        ])
    ]);
    ...
    async function loadPriceAlerts() {
        replace(priceBody, loading());
        const { data, error } = await supabase
            .from('v_price_alerts')
            .select('id, project_id, name, type, district, old_price, new_price, change_percent, changed_at')
            .order('changed_at', { ascending: false })
            .range(0, 49);

        if (!priceBody.isConnected) return;
        if (error) return void replace(priceBody, errorBox(error, 'تعذّر تحميل تنبيهات الأسعار'));
        if (!data || data.length === 0) return void replace(priceBody, empty('لا توجد تغيّرات أسعار حديثة'));
        replace(priceBody, el('div', { class: 'crm-table-wrap' }, priceTable(data)));
    }

    async function loadPriority() {
        replace(priorityBody, loading());
        const { data, error } = await supabase
            .from('v_inventory_priority')
            .select('id, name, type, district, price, employee, completeness_score, priority_score, listing_expires_at')
            .order('priority_score', { ascending: false })
            .order('id', { ascending: true })
            .range(0, 19);

        if (!priorityBody.isConnected) return;
        if (error) return void replace(priorityBody, errorBox(error, 'تعذّر تحميل أولوية العرض'));
        if (!data || data.length === 0) return void replace(priorityBody, empty('لا توجد عقارات جاهزة للعرض'));
        replace(priorityBody, el('div', { class: 'crm-table-wrap' }, priorityTable(data)));
    }

    async function loadGap() {
        replace(gapBody, loading());
        const { data, error } = await supabase
            .from('v_inventory_demand_gap')
            .select('district, open_requests, available_properties, gap, state')
            .order('gap', { ascending: false })
            .range(0, 49);

        if (!gapBody.isConnected) return;
        if (error) return void replace(gapBody, errorBox(error, 'تعذّر تحميل فجوة الطلب'));
        if (!data || data.length === 0) return void replace(gapBody, empty('لا توجد بيانات طلب أو مخزون حسب الحي'));
        replace(gapBody, el('div', { class: 'crm-table-wrap' }, gapTable(data)));
    }

    await Promise.all([loadAttention(), loadDuplicates(), loadPriceAlerts(), loadPriority(), loadGap()]);
}
...
function priceTable(rows) {
    const head = el('thead', {}, el('tr', {}, [
        el('th', { text: 'العقار' }),
        el('th', { text: 'الحي' }),
        el('th', { text: 'السعر السابق' }),
        el('th', { text: 'السعر الجديد' }),
        el('th', { text: 'التغير' }),
        el('th', { text: 'التاريخ' })
    ]));
    const body = el('tbody');
    for (const row of rows) {
        const percent = row.change_percent === null || row.change_percent === undefined
            ? '—'
            : (Number(row.change_percent) > 0 ? '+' : '') + row.change_percent + '%';
        body.appendChild(el('tr', {}, [
            el('td', {}, projectCell(row.name, row.project_id)),
            el('td', { text: dash(row.district) }),
            el('td', { class: 'num', text: money(row.old_price) }),
            el('td', { class: 'num', text: money(row.new_price) }),
            el('td', {}, badge(percent, Number(row.change_percent) < 0 ? 'green' : 'orange')),
...
function priorityTable(rows) {
    const head = el('thead', {}, el('tr', {}, [
        ...
        el('th', { text: 'الحي' }),
        el('th', { text: 'السعر' }),
        el('th', { text: 'اكتمال البيانات' }),
        el('th', { text: 'أولوية العرض' }),
        el('th', { text: 'الموظف' })
    ]));
    const body = el('tbody');
    for (const row of rows) {
        body.appendChild(el('tr', {}, [
            el('td', {}, projectCell(row.name, row.id)),
            el('td', { text: dash(row.type) }),
            el('td', { text: dash(row.district) }),
            el('td', { class: 'num', text: money(row.price) }),
            el('td', {}, badge(Number(row.completeness_score) + '%', Number(row.completeness_score) >= 80 ? 'green' : 'orange')),
            el('td', {}, badge(Number(row.priority_score) + '%', Number(row.priority_score) >= 80 ? 'gold' : 'neutral')),
            el('td', { text: dash(row.employee) })
        ]));
    }
    return el('table', { class: 'users-table crm-table' }, [head, body]);
...
function gapTable(rows) {
    const head = el('thead', {}, el('tr', {}, [
        el('th', { text: 'الحي' }),
        el('th', { text: 'طلبات مفتوحة' }),
        el('th', { text: 'عقارات متاحة' }),
        el('th', { text: 'الفجوة' }),
        el('th', { text: 'الحالة' })
    ]));
    const body = el('tbody');
    for (const row of rows) {
        const tone = row.state === 'عجز' ? 'red' : row.state === 'فائض' ? 'green' : 'neutral';
        body.appendChild(el('tr', {}, [
            el('td', {}, el('strong', { text: dash(row.district) })),
            el('td', { class: 'num', text: String(row.open_requests) }),
            el('td', { class: 'num', text: String(row.available_properties) }),
            el('td', { class: 'num', text: (Number(row.gap) > 0 ? '+' : '') + row.gap }),
            el('td', {}, badge(row.state, tone))
        ]));
    }
    return el('table', { class: 'users-table crm-table' }, [head, body]);
}
